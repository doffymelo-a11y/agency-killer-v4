-- ═══════════════════════════════════════════════════════════════
-- ⚠️ MIGRATION DE SÉCURITÉ CRITIQUE - THE HIVE OS V4 (VERSION 2)
-- ═══════════════════════════════════════════════════════════════
--
-- CORRECTION : Ajout de DROP FUNCTION avant CREATE OR REPLACE
-- pour éviter l'erreur "cannot change return type"
--
-- ═══════════════════════════════════════════════════════════════
...
-- Drop avant recréation pour éviter conflit de signature
...
DROP FUNCTION IF EXISTS is_integration_connected(UUID, integration_type);
...
DROP FUNCTION IF EXISTS get_integration_credentials(UUID, integration_type);
...
DROP FUNCTION IF EXISTS get_project_integrations(UUID);
...
DROP FUNCTION IF EXISTS increment_usage(UUID, VARCHAR, INT);
...
DROP FUNCTION IF EXISTS check_rate_limit(UUID, VARCHAR, VARCHAR);
...
DROP FUNCTION IF EXISTS log_audit(UUID, VARCHAR, VARCHAR, TEXT, JSONB, BOOLEAN, TEXT);
...
DROP FUNCTION IF EXISTS auto_unblock_dependent_tasks();
...
DROP FUNCTION IF EXISTS get_next_task_for_agent(UUID, agent_role);
